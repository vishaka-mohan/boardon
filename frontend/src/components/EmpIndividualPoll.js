...
function EmpIndividualPoll () {
    ...
    useEffect(() => {
        ...
        data => {
            ...
            for(var i = 0; i < data.currPoll.questions.length; i++){
                //console.log(i)
                newAns[data.currPoll.questions[i].qs] = data.currPoll.questions[i].opt[0]
    ...
    return (
        <div className="EmpIndividualPoll" style={{position : 'absolute', top : '15%',left : '20%' }}>
            
            
            {
                (typeof backendData.currPoll === 'undefined') ? ( <p>Loading...</p>) :
                (   
                    <>
                        {
                            console.log(answers)
